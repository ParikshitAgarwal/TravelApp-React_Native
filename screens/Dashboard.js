...
import { dummyData, SIZES, FONTS, COLORS, images, icons } from "../constants";

import { TextButton } from "../components";

const COUNTRIES_ITEMS_SIZE = SIZES.width / 3;
  ...
  function renderCountries() {
    return (
      <Animated.FlatList
        ...
        snapToInterval={COUNTRIES_ITEMS_SIZE}
        ...
        onMomentumScrollEnd={(event) => {
          ...
          var position = (
            event.nativeEvent.contentOffset.x / COUNTRIES_ITEMS_SIZE
          ).toFixed(0);

          //setting the place acc to position
          setPlaces([
            { id: -1 },
        ...
        renderItem={({ item, index }) => {
          const opacity = countryScrollX.interpolate({
            inputRange: [
              (index - 2) * COUNTRIES_ITEMS_SIZE,
              (index - 1) * COUNTRIES_ITEMS_SIZE,
              index * COUNTRIES_ITEMS_SIZE,
            ],
          ...
          if (index === 0 || index === countries.length - 1) {
            return <View style={{ width: COUNTRIES_ITEMS_SIZE }}></View>;
          } else {
            return (
              <Animated.View
                opacity={opacity}
